...
for image_path in images:
    ...
    fm = variance_of_laplacian(image_gray)
    ...
    for i in range(len(scores)):
        if ((scores[i] > min_conf_threshold) and (scores[i] <= 1.0)):
            ...
            #save region of interest images
            ...
            # if the image is lower resolution increase by 5 times
            if((xmax - xmin) > 199):
                img_bar = cv2.resize(img_bar,None,fx=2, fy=2, interpolation = cv2.INTER_CUBIC)
            else:
                img_bar = cv2.resize(img_bar,None,fx=5, fy=5, interpolation = cv2.INTER_CUBIC)
            ...
            for barcode in barcodes:
                ...
                print("[INFO] Found {} barcode: {} loc: {}".format(barcodeType, barcodeData, image_path))
